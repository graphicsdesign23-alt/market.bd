...
export const CustomerCareChat: React.FC = () => {
  ...
  return (
    <>
      ...
      {isCustomerCareOpen && (
        <div className="fixed bottom-14 right-2 left-2 sm:left-auto sm:right-4 z-50 w-auto sm:w-full max-w-sm sm:max-w-md h-[460px] max-h-[75vh] bg-white dark:bg-slate-900 border-2 border-emerald-600 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300 text-slate-900 dark:text-slate-100">
          {/* Header */}
          <div className="bg-gradient-to-r from-emerald-950 via-teal-900 to-slate-900 text-white p-3.5 flex flex-col gap-2 border-b border-emerald-500/40">
              ...
              <div className="flex items-center gap-2.5">
                <div className="relative">
                  <img
                    ...
                    className="w-10 h-10 rounded-2xl object-cover border-2 border-emerald-400"
                  />
          ...
          <div className="bg-slate-100 dark:bg-slate-950 p-2 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between px-1 mb-1 text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">
              ...
              >
                {showFaqList ? (language === 'bn' ? 'লুকান' : 'Hide') : (language === 'bn' ? 'সব প্রশ্ন দেখুন' : 'Show All')}
              </button>
            </div>
          ...
          <div className="p-2 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
            ...
            >
              <input
                ...
                placeholder={
                  language === 'bn'
                    ? 'আপনার প্রশ্ন বিস্তারিত লিখুন...'
                    : 'Type your question...'
                }
                className="flex-1 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 border border-emerald-500 focus:border-emerald-600 rounded-lg text-xs text-slate-900 dark:text-slate-100 font-medium focus:outline-none transition-all"
              />

              <button
                type="submit"
                className="p-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition shadow-md cursor-pointer shrink-0"
                title="Send Message"
              >
                <Send className="w-3.5 h-3.5" />
              </button>
            </form>
            <p className="text-[9px] text-slate-400 dark:text-slate-500 text-center mt-1 font-medium">
              ⚡ 24/7 Official Customer Support • MarketBD.Net Bangladesh
            </p>
          </div>
        </div>
      )}
