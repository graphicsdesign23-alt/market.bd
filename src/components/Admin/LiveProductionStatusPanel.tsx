...
export const LiveProductionStatusPanel: React.FC = () => {
  ...
  // Settings state stored in localStorage for live toggle
  const [isLiveMode, setIsLiveMode] = useState<boolean>(
    localStorage.getItem('marketbd_live_mode') === 'true'
  );

  const [smsProvider, setSmsProvider] = useState<'bulksmsbd' | 'steadfast' | 'greenweb' | 'firebase'>(
    (localStorage.getItem('marketbd_sms_provider') as any) || 'bulksmsbd'
  );

  const [smsApiKey, setSmsApiKey] = useState<string>(
    localStorage.getItem('marketbd_sms_api_key') || ''
  );

  const [smsSenderId, setSmsSenderId] = useState<string>(
    localStorage.getItem('marketbd_sms_sender_id') || 'MarketBD'
  );

  const [gaMeasurementId, setGaMeasurementId] = useState<string>(
    localStorage.getItem('marketbd_ga_id') || 'G-MARKETBD2026'
  );

  const [gscToken, setGscToken] = useState<string>(
    localStorage.getItem('marketbd_gsc_token') || 'MARKETBD_GSC_VERIFICATION_2026'
  );

  // Test states
  const [testPhone, setTestPhone] = useState<string>('01700000000');
  const [testEmail, setTestEmail] = useState<string>('admin@marketbd.net');
  const [testSmsStatus, setTestSmsStatus] = useState<string | null>(null);
  const [testEmailStatus, setTestEmailStatus] = useState<string | null>(null);
  const [savedSuccess, setSavedSuccess] = useState<boolean>(false);

  const handleSaveSettings = () => {
    localStorage.setItem('marketbd_live_mode', isLiveMode ? 'true' : 'false');
    ...
    localStorage.setItem('marketbd_ga_id', gaMeasurementId);
    localStorage.setItem('marketbd_gsc_token', gscToken);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-white space-y-6 shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          ...
          {savedSuccess && (
            <span className="px-3 py-1 bg-emerald-500 text-slate-950 text-xs font-black rounded-lg flex items-center gap-1">
