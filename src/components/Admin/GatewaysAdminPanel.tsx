...
export const GatewaysAdminPanel: React.FC = () => {
  ...
  const [smsProvider, setSmsProvider] = useState<'greenweb' | 'bulksmsbd' | 'steadfast' | 'twilio'>(
    (localStorage.getItem('marketbd_sms_provider') as any) || 'greenweb'
  );
  const [smsApiKey, setSmsApiKey] = useState(localStorage.getItem('marketbd_sms_api_key') || '');
  const [smsSenderId, setSmsSenderId] = useState(localStorage.getItem('marketbd_sms_sender_id') || 'MarketBD');
  const [smsIsLive, setSmsIsLive] = useState(localStorage.getItem('marketbd_live_mode') === 'true');
  const [testPhone, setTestPhone] = useState('01700000000');
  const [smsStatusMsg, setSmsStatusMsg] = useState('');

  // Payment Gateway Config State
  const [paymentLiveMode, setPaymentLiveMode] = useState(localStorage.getItem('marketbd_payment_live_mode') === 'true');
  const [autoSettle, setAutoSettle] = useState(localStorage.getItem('marketbd_payment_auto_settle') !== 'false');
  const [bkashAppKey, setBkashAppKey] = useState(localStorage.getItem('marketbd_bkash_app_key') || '');
  const [nagadMerchantId, setNagadMerchantId] = useState(localStorage.getItem('marketbd_nagad_merchant_id') || '');
  const [sslStoreId, setSslStoreId] = useState(localStorage.getItem('marketbd_ssl_store_id') || '');
  ...
  const handleSaveSmsConfig = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem('marketbd_sms_provider', smsProvider);
    localStorage.setItem('marketbd_sms_api_key', smsApiKey);
    localStorage.setItem('marketbd_sms_sender_id', smsSenderId);
    localStorage.setItem('marketbd_live_mode', String(smsIsLive));
    setSmsStatusMsg(language === 'bn' ? '✓ এসএমএস গেটওয়ে কনফিগারেশন সেভ হয়েছে!' : '✓ SMS Gateway config saved!');
  ...
  const handleSavePaymentConfig = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem('marketbd_payment_live_mode', String(paymentLiveMode));
    localStorage.setItem('marketbd_payment_auto_settle', String(autoSettle));
    localStorage.setItem('marketbd_bkash_app_key', bkashAppKey);
    localStorage.setItem('marketbd_nagad_merchant_id', nagadMerchantId);
    localStorage.setItem('marketbd_ssl_store_id', sslStoreId);
    setPaymentMsg(language === 'bn' ? '✓ অটো পেমেন্ট গেটওয়ে সেটিংস সেভ হয়েছে!' : '✓ Payment Gateway settings saved!');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
          ...
          <form onSubmit={handleSaveSmsConfig} className="space-y-3.5">
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                type="submit"
                className="flex-1 py-2.5 bg-pink-600 hover:bg-pink-700 text-white font-extrabold rounded-xl text-xs shadow-xs transition cursor-pointer"
              >
                {language === 'bn' ? 'সেটিংস সেভ করুন' : 'Save Config'}
              </button>
            </div>
          </form>
        ...
        <div className="p-6 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
          ...
          <form onSubmit={handleSavePaymentConfig} className="space-y-3.5">
            ...
            {paymentMsg && <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 text-center">{paymentMsg}</p>}
          </form>
        </div>
