...
const DEFAULT_SMS_CONFIG: SMSConfig = {
  provider: (localStorage.getItem('marketbd_sms_provider') as any) || 'greenweb',
  apiKey: localStorage.getItem('marketbd_sms_api_key') || '',
  senderId: localStorage.getItem('marketbd_sms_sender_id') || 'MarketBD',
  isLiveMode: localStorage.getItem('marketbd_live_mode') === 'true'
};
...
export async function sendRealSmsOtp(
  ...
  try {
    if (config.provider === 'greenweb') {
      ...
      params.append('From', localStorage.getItem('marketbd_twilio_from') || '+1234567890');
