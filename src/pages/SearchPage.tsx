...
const isCategoryMatch = (productCategory: string, filterCategory: string): boolean => {
  ...
  const targetLower = filterCategory.trim().toLowerCase();
  ...
  const catObj = CATEGORIES.find(c => 
    ...
    c.nameBn.toLowerCase() === targetLower
  );

  const pCatObj = CATEGORIES.find(c => 
    c.id.toLowerCase() === pLower || 
    c.nameEn.toLowerCase() === pLower || 
    c.nameBn.toLowerCase() === pLower
  );
...
const getLocationNamesForProduct = (p: Product) => {
  ...
  const divObj = BANGLADESH_DIVISIONS.find(
    d => d.id.toLowerCase() === divEn.toLowerCase() || d.nameEn.toLowerCase() === divEn.toLowerCase() || d.nameBn === divEn
  );
  if (divObj) {
    divEn = divObj.nameEn;
    divBn = divObj.nameBn;
    if (distEn) {
      const distObj = divObj.districts.find(
        dt => dt.id.toLowerCase() === distEn.toLowerCase() || dt.nameEn.toLowerCase() === distEn.toLowerCase() || dt.nameBn === distEn
      );
      if (distObj) {
        distEn = distObj.nameEn;
        distBn = distObj.nameBn;
        if (thanaEn) {
          const thanaObj = distObj.thanas.find(
            th => th.id.toLowerCase() === thanaEn.toLowerCase() || th.nameEn.toLowerCase() === thanaEn.toLowerCase() || th.nameBn === thanaEn
          );
          if (thanaObj) {
            thanaEn = thanaObj.nameEn;
...
export const SearchPage: React.FC = () => {
    ...
    if (qRaw.length > 0) {
      ...
      tokens.forEach(token => {
        ...
        const matchedCat = CATEGORIES.find(c =>
          ...
          c.nameEn.toLowerCase().includes(token) ||
          c.nameBn.toLowerCase().includes(token) ||
          c.subcategories.some(s => s.id.toLowerCase() === token || s.nameEn.toLowerCase().includes(token) || s.nameBn.toLowerCase().includes(token))
        );
  ...
  const selectedDivisionObj = BANGLADESH_DIVISIONS.find(
    d => d.id === filters.division || d.nameEn.toLowerCase() === filters.division.toLowerCase() || d.nameBn === filters.division
  );
  const selectedDistrictObj = selectedDivisionObj?.districts.find(
    dt => dt.id === filters.district || dt.nameEn.toLowerCase() === filters.district.toLowerCase() || dt.nameBn === filters.district
  );
